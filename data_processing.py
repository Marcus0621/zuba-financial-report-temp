...
def merge_dataset(zuba_result_df, ipay_result_df):
    ...
    df_zuba['TA Commission - 10%'] = df_zuba["Total Room Rate"] * 10/100
    df_zuba['Host Commission - 12%'] = df_zuba["Total Room Rate"] * 12/100
    df_zuba['Total Amount After Commission'] = df_zuba["Total Room Rate"] - df_zuba['Host Commission - 12%']
    ...
    report_order = ['Booking No.', 'Confirmation Code', 'Booking Date', 'User', 'Booker Name', 'Booker Email', 'Check-in Date', 'Check-out Date', 'Property', 'Owner Email', 'Room Type',  
                    ...
                    'ipay88 MDR', 'Payment Method_x', 'Status', 'TA Commission - 10%', 'Host Commission - 12%', 'Total Amount After Commission', 'Total Amount Pay to Host'] 
